...
function parseDimensions(text) {
  if (!text) return null;
  const patterns = [
    /(?:seat\s+height|seat\s+depth|seat\s+width|back\s+height|arm\s+height|width|height|depth|length|diameter|weight)\s*[:=]\s*[\d.,]+\s*(?:cm|mm|kg)/gi,
...
async function main() {
  ...
  logger.info(`Scraping ${products.length} accessories products...\n`);
  ...
  logger.info(`Blank pages (old URLs): ${blank}`);
  logger.info(`No dims on site: ${noDims}`);

  if (blankPages.length) { logger.info(`\nBlank pages:`); blankPages.forEach(n => logger.info(`  - ${n}`)); }
  if (noDimsProducts.length) { logger.info(`\nNo dims on Muuto site:`); noDimsProducts.forEach(n => logger.info(`  - ${n}`)); }
