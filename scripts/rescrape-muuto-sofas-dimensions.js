...
function parseDimensions(text) {
  if (!text) return null;
  const patterns = [
    /(?:seat\s+height|seat\s+depth|seat\s+width|back\s+height|arm\s+height|width|height|depth|length|diameter)\s*[:=]\s*[\d.,]+\s*(?:cm|mm)/gi,
...
async function main() {
  const { rows: products } = await pool.query(
    `SELECT p.id, p.name, p.source_url, p.materials
     FROM products p
     JOIN brands b ON b.id=p.brand_id
     WHERE b.slug='muuto'
       AND p.category='sofas'
       AND (p.dimensions IS NULL OR p.dimensions = '')
     ORDER BY p.name`
  );

  logger.info(`Found ${products.length} sofa products with no dimensions`);
  ...
  for (const product of products) {
    ...
    try {
      ...
      if (dimensions) {
        ...
        if (data.productInfo) logger.warn(`    preview: ${data.productInfo.substring(0, 200)}`);
        noDim.push({ name: product.name, url: product.source_url, preview: data.productInfo?.substring(0, 200) });
        // Still update materials if we got them
  ...
  if (noDim.length) {
    logger.info(`\nNo dimensions found (${noDim.length}):`);
    noDim.forEach(p => logger.info(`  - ${p.name}\n    url: ${p.url}\n    preview: ${p.preview || 'null'}`));
  }
  if (failed.length) {
