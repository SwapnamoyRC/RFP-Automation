...
function parseDimensions(text) {
  if (!text) return null;
  const patterns = [
    /(?:seat\s+height|seat\s+depth|width|height|depth|length|diameter|weight|cord\s+length)\s*[:=]\s*[\d.,]+\s*(?:cm|mm|kg)/gi,
...
async function main() {
  const { rows: products } = await pool.query(`
    ...
    WHERE b.slug = 'muuto' AND p.category = 'lighting'
      AND (p.dimensions IS NULL OR p.dimensions = '')
    ORDER BY p.name
  `);

  logger.info(`Scraping ${products.length} lighting products...\n`);
